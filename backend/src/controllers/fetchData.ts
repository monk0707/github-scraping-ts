...
import { batchPipelineSelenium } from "./fetchSocialMediaData";

async function normalFetch(url: string) {
    try {
...
async function socialMediaFetch(url: string) {
    try {
...
export const batchPipelineFetch = async function batchPipelineFetch(urlsToScrape: string[]) {
    ...
    const responses = await Promise.all(otherUrls.map((url) => normalFetch(url)));
    ...
    return { responses, socialMediaResponses };
}
